...
print(f"  {'Metric':<28} {'Linear Reg':>12} {'Logistic Reg':>14} {'Random Forest':>14}")
print(f"  {'-'*28} {'-'*12} {'-'*14} {'-'*14}")
print(f"  {'R²':<28} {r2:>12.4f} {'—':>14} {'—':>14}")
print(f"  {'MAE':<28} {mae:>12.4f} {'—':>14} {'—':>14}")
print(f"  {'RMSE':<28} {rmse:>12.4f} {'—':>14} {'—':>14}")
print(f"  {'Accuracy':<28} {'—':>12} {m_log['accuracy']:>14.4f} {m_rf['accuracy']:>14.4f}")
print(f"  {'Precision (Pass)':<28} {'—':>12} {m_log['precision']:>14.4f} {m_rf['precision']:>14.4f}")
print(f"  {'Recall (Pass)':<28} {'—':>12} {m_log['recall']:>14.4f} {m_rf['recall']:>14.4f}")
print(f"  {'F1 Score (Pass)':<28} {'—':>12} {m_log['f1']:>14.4f} {m_rf['f1']:>14.4f}")
print(f"  {'CV Score (5-fold)':<28} {cv_r2.mean():>12.4f} {cv_log.mean():>14.4f} {cv_rf.mean():>14.4f}")
print(f"  {'CV Std Dev':<28} {cv_r2.std():>12.4f} {cv_log.std():>14.4f} {cv_rf.std():>14.4f}")
print()
...
print()
print(f"  {'Metric':<35} {'Logistic Reg':>14} {'Random Forest':>14}")
print(f"  {'-'*35} {'-'*14} {'-'*14}")
print(f"  {'Fail Precision':<35} {fp_log:>14.4f} {fp_rf:>14.4f}")
print(f"  {'Fail Recall (at-risk detection rate)':<35} {fr_log:>14.4f} {fr_rf:>14.4f}")
print(f"  {'Fail F1':<35} {ff_log:>14.4f} {ff_rf:>14.4f}")
print(f"  {'Failing students correctly caught':<35} {tn_l:>14} {tn_r:>14}")
print(f"  {'Failing students missed':<35} {fn_l:>14} {fn_r:>14}")
print()
...
ax.plot([0, 1], [0, 1], color="#C44E52", linestyle="--",
...
plt.tight_layout()
plt.savefig("../plots/plot11_roc_curves.png")
plt.show()
print("Plot 11 saved: plot11_roc_curves.png")
...
baseline_pr = y_clf_test.mean()  # proportion of positives = baseline AP
...
ax.axhline(y=baseline_pr, color="#C44E52", linestyle="--", linewidth=1.2,
           label=f"Baseline (AP = {baseline_pr:.3f})")
ax.set_title("Precision-Recall Curves — Logistic Regression vs Random Forest",
...
plt.tight_layout()
plt.savefig("../plots/plot12_precision_recall.png")
plt.show()
print("Plot 12 saved: plot12_precision_recall.png")
print()
...
print()
print("  3. Logistic Regression outperformed Random Forest on this dataset.")
print("     With only 395 students and weak non-linear signals, a simpler")
print("     model generalised better. Random Forest showed higher CV")
print(f"     consistency (F1 CV = {cv_rf.mean():.2f} vs {cv_log.mean():.2f}) but lower test F1.")
print()
...
report = pd.DataFrame([
    {
        ...
        "Recall (Pass)":     "—",
        "F1 (Pass)":         "—",
        "CV Score (mean)":   round(cv_r2.mean(), 4),
        "CV Score (std)":    round(cv_r2.std(), 4),
        "AUC-ROC":           "—",
    },
    {
        ...
        "Precision (Pass)":  round(m_log["precision"], 4),
        "Recall (Pass)":     round(m_log["recall"], 4),
        "F1 (Pass)":         round(m_log["f1"], 4),
        "CV Score (mean)":   round(cv_log.mean(), 4),
        "CV Score (std)":    round(cv_log.std(), 4),
    ...
    },
    {
        ...
        "Precision (Pass)":  round(m_rf["precision"], 4),
        "Recall (Pass)":     round(m_rf["recall"], 4),
        "F1 (Pass)":         round(m_rf["f1"], 4),
        "CV Score (mean)":   round(cv_rf.mean(), 4),
        "CV Score (std)":    round(cv_rf.std(), 4),
        "AUC-ROC":           round(auc_rf, 4),
    },
])

report.to_csv("../results/results_final_report.csv", index=False)



print("Files saved:")
print("  plot11_roc_curves.png")
print("  plot12_precision_recall.png")
print("  results_final_report.csv")